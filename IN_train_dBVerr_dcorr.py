...
for i in range(num_epochs):
    ...
    for j in range(int(len(train[0])/batch_num)):
        ...
        batch_ntk = train[1][j*batch_num:(j+1)*batch_num][:,2]
    ...
    for j in range(int(len(val[0])/batch_num)):
        ...
        batch_ntk = val[1][j*batch_num:(j+1)*batch_num][:,2]
        batch_ntk = np.reshape(batch_ntk, (-1,1))
...
with tf.Session() as newsess:
    ...
    for j in range(int(len(test[0])/batch_num)+1):
        ...
        batch_ntk = test[1][j*batch_num:next_idx][:,2]
...
t_5tkh = truth[(pred>0.4) & (ntk<=0.0025)]
t_5tkl = truth[(pred<0.4) & (ntk<=0.0025)]
t_4tkh = truth[(pred>0.4) & (ntk>0.0025)]
t_4tkl = truth[(pred<0.4) & (ntk>0.0025)]
t_3tkh = truth[(pred>0.4) & (ntk>0.0025)]
t_3tkl = truth[(pred<0.4) & (ntk>0.0025)]
